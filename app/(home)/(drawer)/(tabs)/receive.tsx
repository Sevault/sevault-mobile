...
const ShareableAddressCard = ({
  ...
  return (
    <View style={styles.addressCard} ref={ref}>
      ...
      <View style={{ marginTop: actuatedNormalizeVertical(20) }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Text style={{ color: '#888888', fontWeight: 'bold' }}>Network</Text>
      ...
      <View style={{ marginTop: actuatedNormalizeVertical(20) }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', flexWrap: 'wrap' }}>
          <Text style={{ color: '#888888', fontWeight: 'bold' }}>Wallet Address</Text>
      ...
      <View style={{ paddingTop: actuatedNormalizeVertical(30), paddingBottom: actuatedNormalizeVertical(30) }}>
        <Text style={{ color: '#888888', }}>
          Only send <Text style={{ fontWeight: 'bold' }}>{asset}</Text> (<Text style={{ fontWeight: 'bold' }}>{network}</Text>) to address.
        </Text>
      </View>

      <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
        <Image
          style={{ height: actuatedNormalizeVertical(48), width: actuatedNormalize(134) }}
...
const ReceiveBGL = () => {
  ...
  const onShare = async () => {
    ...
    try {
      const result = await Share.share({
        message:
          `Send ${asset} (${network}) to this address: ${address}. Shared from SevaultWallet.`,
      })

      if (result.action === Share.sharedAction) {
        if (result.activityType) {
          displayToast({ message: `Sucesfully Shared wallet address`, type: 'info' })
        } else {
          displayToast({ message: `Sucesfully Shared wallet address`, type: 'info' })
        }
  ...
  const onSaveImageAsync = async () => {
    try {
      ...
      if (localUri) {
        displayToast({ message: 'Saved to Gallery', type: 'info' })
      }
    } catch (e) {
      displayToast({ message: 'Failed to save image', type: 'info' })
    }
  }
  ...
  return (
    <ScrollView style={styles.receiveContainer}>
      ...
      <View style={styles.addressCard} ref={imageRef} collapsable={false}>
        <View style={{ alignItems: 'center', justifyContent: 'center' }}>
          <QRCode
            size={150}
            value={address as string}
      ...
      <View style={{ borderRadius: actuatedNormalize(20) }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <Pressable
          ...
          onPress={() => {
            ...
            onSaveImageAsync()
            onShare()
            // save image to gallery with the QR Code, will need call to sscreenshot api? refer to figma design
            // show a toas at the bottom(android native style)
          }}
        >
          <Text style={{ color: COLORS.WHITE, fontWeight: '600', fontSize: actuatedNormalize(16) }}>
            Save & Share
          </Text>
        </Pressable>
      </View>
